        ...
        <div class="bg-white px-6 py-8 rounded text-black w-full">
            ...
            @foreach ($randomize as $item)
                ...
                @foreach ($answers as $ans)
                    ...
                    <input type="radio" name={{'choice'.$item->quest_id}} value="{{ $ans->answ_title }}"> {{ $ans->answ_title }} <br>
               
                    @endforeach

            @endforeach
            <button type="submit"
            class="w-full mt-4 text-center py-3 rounded bg-rose-600 text-white hover:bg-green-dark focus:outline-none my-1">Save & Confirm my quiz</button>
